...
%% 数据读入--------------------------------------------------------
MAP = xml2struct( 'opendrive_demo_20190516.xml' );                          % 读取 OpenDRIVE 文件（xml格式）
...
for i = 1:1:road_num
    ...
    id = str2num(id);                                                       % 读取road id
    ROADS(id).id = id;                                                      % 写入road id
    
    % road length----------------------------------------------------------
    ROADS(id).length = str2num(MAP.road{1,i}.Attributes.length);            % 写入road length
    
    % Start_x, Start_y-----------------------------------------------------
    geometry_num = size(MAP.road{1,i}.planView.geometry);
    geometry_num = geometry_num(1,2);
    if geometry_num ==1
        ROADS(id).start_x = str2num(MAP.road{1,i}.planView.geometry.Attributes.x);  % road起点的 x 坐标
        ROADS(id).start_y = str2num(MAP.road{1,i}.planView.geometry.Attributes.y);  % road起点的 y 坐标 
    elseif geometry_num > 1
        ROADS(id).start_x = str2num(MAP.road{1,i}.planView.geometry{1,1}.Attributes.x);  % road起点的 x 坐标
        ROADS(id).start_y = str2num(MAP.road{1,i}.planView.geometry{1,1}.Attributes.y);  % road起点的 y 坐标  
    end;
